...
class Polygone {
    ...
    constructor(pointA, nbrArrete, rayon) {
        ...
        this.couleurPolygone="white";
        this.actualiseInfoPolygone()
    }
    changerCouleurPolygone(couleur){
    ...
    actualiseInfoPolygone() {
        ...
        for (var i = 0; i < this.nbrArrete; i++) {
            ...
            this.listePointPolygone.push(point_a)
            angle180 += Math.PI * 2 / this.nbrArrete
            if (i > 0 && this.nbrArrete >= 2) {
                this.listeCerclePolygone.push(new CercleDeCentre_milieuSegment(this.listePointPolygone[i - 1], this.listePointPolygone[i]))
                this.listeTrianglePolygone.push(new Triangle(this.pointOrigine, this.listePointPolygone[i - 1], this.listePointPolygone[i]))
            }
        }
        if (this.nbrArrete >= 2) {
            this.listeCerclePolygone.push(new CercleDeCentre_milieuSegment(this.listePointPolygone[this.nbrArrete - 1], this.listePointPolygone[0]))
            this.listeTrianglePolygone.push(new Triangle(this.pointOrigine, this.listePointPolygone[this.nbrArrete - 1], this.listePointPolygone[0]))
        } else {
            this.listeCerclePolygone.push(new CercleDeCentre_milieuSegment(this.pointCentrePolygone, this.listePointPolygone[0]))
        }
        this.listeCerclePolygone.forEach((arrr) => {
            this.listeArretePolygone.push(arrr.segmentPere)
        })
        if (this.nbrArrete == 2) {
            this.cercleInscritPolygone = new CercleDeRayon_moitierSegment(this.pointOrigine, this.listePointPolygone[0]);
            this.cercleCirconscritPolygone = new CercleDeRayon_longueurSegment(this.pointOrigine, this.listePointPolygone[0]);
        } else if (this.nbrArrete == 1) {
            this.cercleCirconscritPolygone = new CercleDeCentre_milieuSegment(this.pointOrigine, this.listePointPolygone[0]);
     ...
     * @param {isAvecCouleur} Boolean
     */
    dessinerPolygone(isAvecCouleur=false) {
        this.listeArretePolygone.forEach((arr) => {
            arr.dessinerSegment()
        })
        if(isAvecCouleur){
            crayon.beginPath()
            crayon.fillStyle=this.couleurPolygone
            crayon.moveTo(this.listePointPolygone[0].x,this.listePointPolygone[0].y)
            this.listePointPolygone.forEach((pou1,ind)=>{
                if(ind!=0){
                    crayon.lineTo(pou1.x,pou1.y)
                }
            })
            crayon.fill()
        }
...
function demo() {
    if(true){
        ...
        //-------------------------------------
        var p1=new Point("p1",0,poli_p.pointCentrePolygone.y)
        var sseg1=new Segment(p1,poli_p.pointCentrePolygone)

        var p2=new Point("p2",canvas.width,poli_p.pointCentrePolygone.y)
        var sseg2=new Segment(p2,poli_p.pointCentrePolygone)

        var p3=new Point("p3",poli_p.pointCentrePolygone.x,0)
        var sseg3=new Segment(p3,poli_p.pointCentrePolygone)

        var p4=new Point("p4",poli_p.pointCentrePolygone.x,canvas.height)
        var sseg4=new Segment(p4,poli_p.pointCentrePolygone)

        poli_p.cercleCirconscritPolygone.changerColorArc("green")
        poli_p.cercleCirconscritPolygone.afficherCouleurCercle()
        poli_p.cercleCirconscritPolygone.dessinerArc()

        poli_p.changerCouleurPolygone("black")
        poli_p.dessinerCentrePolygone()
        poli_p.tournerPolygone(0.02)
        poli_p.dessinerPolygone(true)
        
        poli_p.cercleInscritPolygone.changerColorArc("yellow")
        poli_p.cercleInscritPolygone.afficherCouleurCercle()
        poli_p.cercleInscritPolygone.changerColorContourArc("red")
        poli_p.cercleInscritPolygone.dessinerArc()

        sseg1.dessinerSegment()
        sseg2.dessinerSegment()
        sseg3.dessinerSegment()
        sseg4.dessinerSegment()
        


        //-------------------------------------
        var p = f1.creer_point(souris.mouseX, souris.mouseY, "Rolio")
        p.afficherPosition(true)
        p.afficherNom(false)
        p.changerCouleur("red")
        p.changerTaillePoint(5)
        var ss=new Segment(p,poli_p.pointCentrePolygone)
        if (souris.isclicked) {
            //p.dessinerPoint()
            ss.dessinerSegment()
        }
        poli_p.listePointPolygone.forEach((point)=>{
            var l=new Polygone(point,5,50)
            var s1=new Segment(p,l.pointCentrePolygone)
            l.tournerPolygone(iii)
            l.listePointPolygone.forEach((po)=>{
                po.afficherNom(false)
                var sef=new Segment(po,poli_p.pointCentrePolygone)
                sef.dessinerSegment()
            })
            l.changerCouleurPolygone("blue")
            l.dessinerPolygone(true)
            s1.dessinerSegment()
        })
